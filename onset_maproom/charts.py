...
def onset_date(lat, lng, params):
    return dlchart("905cdac6e87a58586967e115a18e615d01530ddd", lat, lng, params)

def prob_exceed(lat, lng, params):
    return dlchart("cfd4d8c8f8d13dbbbdbde213cc4bed4dfde2eb30", lat, lng, params)

def cess_date(lat, lng, params):
    return dlchart("f21ac05490eb01801481b2f939c5de6ceade3cd9", lat, lng, params)
...
def table(lat, lng, params):
    ...
    onset_data = onset_json['iridl:values']
    cess_data = cess_json['iridl:values']
    return [ [onset_data[i]['T'], onset_data[i]['min'], cess_data[i]['min'], ] for i in range(len(onset_data)) ]
